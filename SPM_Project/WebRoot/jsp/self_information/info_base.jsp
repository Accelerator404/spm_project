...
$(document).ready(function() {
	...
	$('#edit').click(function name() {
		$("#txtdiv").attr("hidden",true)
		$("#editdiv").removeAttr("hidden")
		getinfo_edit();
	})
	
	$('#discard').click(function name() {
		$("#editdiv").attr("hidden",true)
		$("#txtdiv").removeAttr("hidden")
	})
	
	$('#save').click(function name() {
		setinfo();//保存修改
		
	}) 
	
})
function getinfo_txt(){
	var url="${ctx}/getUserInfo.do";
	var data={"userName":"${session.user.userName}"};
	$.ajax({
		url:url,
		data:data,
		type:"GET",
		...
		success:function(data){
			console.log(data)
			var Gender=['男','女']
			var Identity=['在校学生','在职','其他']
			var MaxDegree=['大专','本科','硕士','博士']
			var G = Gender[parseInt(data['gender'])-1]
			var i = Identity[parseInt(data['identity'])-1]
			var m = MaxDegree[parseInt(data['maxDegree'])-1]
			toTxt=""
			toTxt+="<div><lable>用户名:  		</lable>"+"<span>"+${session.user.userName}+"</span></div>"
			toTxt+="<div><lable>昵称:  		</lable>"+"<span>"+data['nickname']+"</span></div>"
			toTxt+="<div><lable>性别:  		</lable>"+"<span>"+G+"</span></div>"
			toTxt+="<div><lable>身份: 		</lable>"+"<span>"+i+"</span></div>"
			toTxt+="<div><lable>学校:  		</lable>"+"<span>"+data['school']+"</span></div>"
			toTxt+="<div><lable>学历:  		</lable>"+"<span>"+m+"</span></div>"
			toTxt+="<div><lable>介绍:  		</lable>"+"<span>"+data['selfIntroduction']+"</span></div>"
...
function getinfo_edit(){
	...
	$.ajax({
		...
		success:function(data){
			...
			$("#gender").val(data['gender']);//1,2
			$("#identity").val(data['identity']);//1,2,3
			$("#schoolName").val(data['school']);
			$("#maxDegree").val(data['maxDegree']);
			$("#selfIntroduction").val(data['selfIntroduction']);	
		}
	})
}
function setinfo(){
	var url="${ctx}/userInfoAdd.do";
	var userName = ${session.user.userName};
	var gender = $("#gender").val();var identity = $("#identity").val();var maxDegree = $("#max_degree").val()
	var nickname = $("#nickname").val();var school = $("#schoolName").val();var selfIntroduction = $("#selfIntroduction").val()
	var postdata={"userName":userName,"gender":gender,"identity":identity,"maxDegree":maxDegree,"nickname":nickname,"school":school,"selfIntroduction":selfIntroduction}
	$.ajax({
		url:url,
		type:"POST",
		data:postdata,
...
function adit() {
	...
	$("#selfIntroduction").removeAttr("disabled");
	$("#schoolName").removeAttr("disabled");
	$("#gender").removeAttr("onfocus");
	$("#gender").removeAttr("onchange");
	$("#max_degree").removeAttr("onfocus");
	$("#max_degree").removeAttr("onchange");
	$("#identity").removeAttr("onfocus");
	$("#identity").removeAttr("onchange");
}
</script>
<style type="text/css">
	  ul li{ list-style:none;}
	  #schoolName { width:200px;}
	  .provinceSchool { display:none;position:absolute;width:580px;height:340px;border:1px solid #72B9D7; }
	  ...
	  .provinceSchool .button button { float:right;height:30px;margin-right:20px;padding:4px 10px; background:url(images/title-bg.png);border:none;color:#FFF;font-weight:600;cursor:pointer;}</style>

</head>
<body>
<div class="content" style="margin-left: 50px;padding-left:30px; border-left: 1.5px dashed black;">
<div>
<h1 style="color:navy;">个人信息资料</h1><button type="button" class="btn btn-success" id='edit' onclick="edit();">编辑</button>
...
<label class="required-input" for="username">用户名</label>
<div class="input-row">
         <input type="text" id="username" disabled="disabled" value="${session.user.userName }">
     </div>
<br>
<br>
<div class="input-row">
     <label class="required-input" for="nickname"><span style="color:red;">*</span>昵称</label>
    ...
    <select class="form-control" id="gender" style="width:84px;">
    ...
    <select class="form-control" id="identity" style="width:130px;" >
     ...
     <br>
     <label for='schoolName'> 你的学校名称：</label><input type="text" id="schoolName" readonly  />
    <div id="proSchool" class="provinceSchool">
      ...
      <div class="button">
      	<button type="button" flag='0' class="btn btn-info">取消</button>
        <button type="button" flag='1' class="btn">确定</button>
      </div>
    </div>
    <br>
    <br>
     <div class="form-group">
        <label for="max_degree">最高学历</label>
        <select id="max_degree" style="width:84px;" class="form-control">
    ...
    <form role="form">
       <div class="form-group">
         <label for="selfIntroduction" style="display:block;margin-bottom: 5px;">个人简介</label>
         <textarea class="form-control" rows="4" placeholder="请简单介绍下自己,不超过100字" id="selfIntroduction" style="width:360px;"></textarea>
       </div>
    </form> 
    <br>
    <div class="btn-group">
         <button type="button" class="btn btn-warning" onclick="javascript:return false;" id="discard">放弃修改</button>
         <button type="button" class="btn btn-info" style="margin-left: 20px;" id="save">确定修改</button>
    </div> 
</div>
</div>
</div>
